...
#define XEMACPS_SEND_BD_CNT     252
#define XEMACPS_RECV_BD_CNT     252
#define XEMACPS_SEND_SKB_CNT    42
#define XEMACPS_RECV_SKB_CNT    126
...
static int xemacps_SetRxBD(struct net_device *ndev)
{
    struct net_local *lp;
    struct xemacps_bd *cur_p;
    struct buff *buff_pos, *buff_next;
    u32 offset[3] = {6680, 5184, 4080};
    int i, j;
    u32 bdidx, regval;

    lp = (struct net_local *)netdev_priv(ndev);
    xemacps_dbg("rxbd freecnt = %d\n", lp->rx_bd_freecnt);
    bdidx = lp->rx_bd_tail;

    for(i = 0; i < lp->rx_bd_freecnt / 3; i++)
    {
        // prepare rx buffer for bd
        buff_pos = list_entry(lp->pos, struct buff, list);
        do
        {
            lp->pos = lp->pos->next;
            buff_next = list_entry(lp->pos, struct buff, list);
        }while(buff_next->stat == 1);
        xemacps_dbg("bd: %2d, %2d, %2d, buff->addr: %#x, buff->stat: %d\n", bdidx, bdidx + 1, bdidx + 2, buff_pos->addr, buff_pos->stat);

        for(j = 0; j < 3; j++)
        {
            cur_p = &lp->rx_bd[bdidx + j];
            regval = ((buff_pos->addr + offset[j]) & XEMACPS_RXBUF_ADD_MASK);
            /* set wrap bit for last BD */
            if (bdidx + j == XEMACPS_RECV_BD_CNT - 1) 
    ...
    lp->rx_bd_freecnt -= 3 * i;

    lp->rx_bd_tail = (lp->rx_bd_tail + 3 * i) % XEMACPS_RECV_BD_CNT;
    return 0;
}
...
static int xemacps_rx(struct net_local *lp, int budget)
{
    ...
    while (numbdfree < budget) {
        ...
        paddr = cur_p->addr & XEMACPS_RXBUF_ADD_MASK;
        rmb();
        xemacps_dbg("bd%d paddr: %#x, len: %d\n", lp->rx_bd_ci, paddr, len);

        packets++;
        lp->rx_package ++;

        if(len == cmd_len + mac_len || len == large_cmd_len + mac_len)
        {
            lp->asend.cmd_parse(paddr + mac_len, len, lp->rx_bd_ci);
        }
        else if( (len == 1512) || (len == 1120) )
        {
            // these packets may come from multiple xemacps_rx
            rx_size += len - mac_len;
            if(rx_size > block_len) rx_size -= block_len;
            if(len == 1120)
            {
                paddr_align = paddr;
                lp->asend.data_recv(paddr_align + mac_len, rx_size, lp->rx_bd_ci);
            }
        }
        else 
        {
            xemacps_dbg("receive other package\n");
        }
    ...
    lp->rx_bd_freecnt += numbdfree;

    lp->stats.rx_packets += packets;
    lp->stats.rx_bytes += rx_size;
    return numbdfree;
}
...
static int xemacps_descriptor_init(struct net_local *lp)
{
    ...
    lp->rx_bd_freecnt = XEMACPS_RECV_BD_CNT;
    // for (i = 0; i < XEMACPS_RECV_BD_CNT; i++) {
    //     cur_p = &lp->rx_bd[i];
...
int mac_str_to_bin( char *str, u8 *mac,int j)
{
    int i;
    char *s, *e;
...
static int xemacps_open(struct net_device *ndev)
{
    ...
    // init mac header
    size = 16;
    lp->mac = dma_alloc_coherent(&lp->pdev->dev, size,
            &lp->mac_mapping, GFP_KERNEL);
    if (!lp->mac)
        return -1;
    dev_dbg(&lp->pdev->dev, "mac address %d bytes at 0x%x mapped %p\n",
            size, lp->mac_mapping, lp->mac);

    // register asend_op
    lp->asend.net_buff_send = net_buff_send;
    lp->asend.xemacps_SetRxBD = xemacps_SetRxBD;


    lp->rx_package = 0;
    init_waitqueue_head(&lp->rev_irp);
...
int net_buff_send(u32 paddr, u32 size, struct net_device *ndev)
{
    int frag_len[3] = {1496, 1496, 1104};
    u32 faddr[3];
    int i;
    unsigned int nr_frags = 2, len;
    u32 regval, bd_tail, nr_page, sent_page = 0;
    struct xemacps_bd *cur_p;
    unsigned long flags;
    struct net_local *lp = netdev_priv(ndev);

    char *src_mac = "00:0a:35:00:01:22";
    char *dst_mac = "00:0a:35:00:01:22";
    char *type_mac = "08:00:00:01";
    mac_str_to_bin(dst_mac, lp->mac, 6);
    mac_str_to_bin(src_mac, lp->mac + 6, 6);
    mac_str_to_bin(type_mac, lp->mac + 12, 4);

    faddr[0] = paddr + 1104 + 1496;
    faddr[1] = paddr + 1104;
    faddr[2] = paddr;

    if(size <= large_cmd_len)
    {
        if (nr_frags > lp->tx_bd_freecnt) 
        {
            netif_stop_queue(ndev); /* stop send queue */
    ...
    else
    {
        ...
        while(nr_page)
        {
            for (i = 0; i < 3; i++) 
            {
                ...
                {
                    netif_stop_queue(ndev); /* stop send queue */
                    xemacps_dbg("no enough tx bd freecnt\n");
                    return NETDEV_TX_BUSY;
                }
                ...
                //**************************set of data BD************************
                len = frag_len[i];
                cur_p->addr = faddr[i] + 4096 * sent_page;
                /* preserve critical status bits */
                regval = cur_p->ctrl;
                regval &= (XEMACPS_TXBUF_USED_MASK | XEMACPS_TXBUF_WRAP_MASK);
                /* update length field */
...
static int xemacps_probe(struct platform_device *pdev)
{
    ...
    dev_info(&lp->pdev->dev, "ethernet compatible: %s\n", (const char *)prop);
    ...
    if (mac_address) {
        memcpy(lp->ndev->dev_addr, mac_address, 6); // pp->length == 6
        xemacps_set_hwaddr(lp);
    } else {
